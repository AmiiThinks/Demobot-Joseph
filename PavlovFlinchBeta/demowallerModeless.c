...
float hit_decay=0;
//--------------- Modeless support
float pdecision(float prediction, float pa, float da, float pb, float db) {
  //define a piecewise linear decision function, from da at pa to bd at pb.  float d;
  float d=0;
  if (prediction < pa) 
    d=da;
  else if (prediction >= pb)
    d=db;
  else
    d= (db-da)* (1./(pb-pa))*(prediction-pa);

  return d;
}

// in .6 seconds (20 timesteps).  Maybe scale to 0.1 s 
bool probTurnOnLastForward(float prediction) {
  float pa=0.4, da=0, pb=.75, db=.8; 
  float d=pdecision(prediction,pa,da,pb,db);
  return  d > uniform_random();
}


bool probTurnOnLastTurn(float prediction) {
  float pa=0.3, da=.1, pb=.7, db=.95; 
  float d=pdecision(prediction,pa,da,pb,db);
  return  d > uniform_random();
}
...
int weighted_preferences_policy() {
  ...
  //printf("T: %4.3f pred:%4.3f \t prefs: %5.3f %5.3f %5.3f  \t wts:%5.3f %5.3f %5.3f  roll: %4.3f  sel: %d a: %d out: %d\n", T, p0, pref[0],pref[1],pref[2],weights[0],weights[1],weights[2],roll,selection,action_prime,out);
...
int agentPolicy(int s) {
  ...
  //printf("p0:% 5.3f p1:% 5.3f p2:% 5.3f last_action:%d\n", predictions[0], predictions[1],predictions[2],action_prime);
...
void agentUpdate(int obs, int act,int pktNum) { //post policy
  ...
  double netTime = time_delta+ (timeNow.tv_usec*1.-timeBegin.tv_usec)/1000000.;
    ...
    if (startingLights >0) {
      ...
      setLEDs(0,255, 1 , 1);
    else if (action_prime == RAND_LEFT)  
      setLEDs(0,0, 1 , 0);
    else if (action_prime == RAND_RIGHT)  
      setLEDs(0,0, 0 , 1);
    else
      setLEDs(0,0, 0 , 0);
    //printf("nettime: %f %ld %ld\n",netTime, timeNow.tv_sec, timeBegin.tv_sec);
    if (action_prime==EXPLORE_MODE ) {
      //setLEDs(255,255, 0 , 0);
      setExtLEDs( ((iteration/3) %2)==1 ,  ((iteration/5) %2)==0 ,  ((iteration/5) %2)==0);
    } else {
      //setLEDs( 0 ,255, obs !=0 , prediction > .4);
      setExtLEDs(predictions[0] > .358 ,  predictions[0] > .505 ,predictions[0] > .18 );
    }
    //int vc=vid_count%2;
    ...
    //  rs[k]=obs > 0 ? 1 : 0;
    //  gammas[k]=rs[k]>0 ? 0 : .95;
    //  rhos[k]=last_act==FORWARD ? 1: 0;
    //}
    ...
    /*
    float beta= .25;
    rs[1] = beta*predictions[0];//obs > 0 ? 1 : 0;
    gammas[1]=1-beta;
    rhos[1]=last_act==COUNTERCLOCKWISE ? 1: 0;

    rs[2] = beta*predictions[0];//obs > 0 ? 1 : 0;
    gammas[2]=1-beta;
    rhos[2]=last_act==CLOCKWISE ? 1: 0;
    for (int k=0;k<DEMONS;k++) 
...
void init_logging() {
  ...
  char codename[]="demowallerD1";
  sprintf(logfilename,"mkdir ../logfiles/%s-y%d-m%02d-d%02d-h%02d-m%02d-s%02d.src", codename,
  ...
  sprintf(logfilename,"cp Makefile *.c *.h ../logfiles/%s-y%d-m%02d-d%02d-h%02d-m%02d-s%02d.src", codename,
          (tB->tm_year+1900),(1+tB->tm_mon),tB->tm_mday,
           tB->tm_hour,tB->tm_min,tB->tm_sec);
  ...
  sprintf(logfilename,"../logfiles/demoWallerD-y%d-m%02d-d%02d-h%02d-m%02d-s%02d.logfile",
          (tB->tm_year+1900),(1+tB->tm_mon),tB->tm_mday,
           tB->tm_hour,tB->tm_min,tB->tm_sec);
