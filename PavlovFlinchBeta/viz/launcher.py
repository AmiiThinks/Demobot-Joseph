...
# Now try with a pool (from http://stackoverflow.com/questions/26206102/non-blocking-hostname-resolving)
...
def poolcheck(names):
    pool = mp.Pool(processes=5,maxtasksperchild=2)
    results = [pool.apply_async(lookup, (n,)) for n in names]
    done = 0
    vals=[]
    for r in results:
        try:
...
#     ret = subprocess.call(["/sbin/ping", "-c", "1", name],stderr=subprocess.PIPE, stdout=subprocess.PIPE)
#     return ret==0 #
#     #return 0==os.system("ping -c 1 " +name)
...
def startup(name):
    subprocess.call(["./terminalIt","ipython", "--pylab","osx","viewer.py", name])
...
class MyTable(tk.Frame):
    def __init__(self,parent,columns=4):
        ...
        self.machines=["ras1.local","ras2.local","ras3.local","ras4.local","ras5.local","ras6.local","ras20.local","ras21.local","ras22.local","ras23.local"]#"pij.local","berry.local"]
    ...
    def update_me(self):
        ...
        comms=poolcheck(self.machines)
        for i in range(len(self.machines)):     
            name=self.machines[i]
            #comm=pingmachine(name)
            self.set(i+1,1,"%s"%comms[i])
            alive=netreadline(name) if comms[i] else None 
            self.set(i+1,2,"%s"%alive)
            if alive:
                self.widgets[i+1][3].config(state=tk.NORMAL)
            else:
