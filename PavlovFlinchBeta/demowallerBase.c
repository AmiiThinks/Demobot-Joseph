...
void updateLogfile(int obs, int act, int pktNum){
  ...
  {//logging
    ...
    if (action_prime!= STOP)
      iteration++;
    c+=sprintf(logstring+c," %d %d %d %d",era, learn_prediction, use_prediction, policy_type);
...
void init_logging() {
  ...
  loc += sprintf(start_comment+loc," era learn_prediction use_prediction policy_type");
