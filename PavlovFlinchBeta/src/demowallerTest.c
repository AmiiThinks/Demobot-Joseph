//tests
// 1. throttle output to 30ms on unchanged commands - this fixes
// the latency on the Create2.  It is not so simple to describe in a
// paper though, so it might be simplified for simpler writeups.

// Remaining
// 2. print comparison of modeless with actor critic to find the error.
//    --- failed to initialize learning due to -create2 command line argument.
//    ---  should be fixed now.
//    --- should only call ActorCritic Update when it is in control.
// 3. compare a few reward-maximizing runs to non-maximizing runs
// 4. Fix ras22 SD card and try to autofix partition problems?
// .. Run on create.v1 and compare motor-sensor latencies
...
const char logfile_prefix[]="/home/pi/logfiles/demowallerTest";
...
const float decisions_per_second=100/3.;
...
int policy_type=MODELESS;//ACTOR_CRITIC;
...
int testing=0;
const float testing_probability=0.1;
...
float mem_trace_turning=0;
const float mem_trace_turning_Ts=3;
const float mem_trace_turning_gamma= 1-1.0/(mem_trace_turning_Ts * decisions_per_second);


//remote commands from display
bool req_play=false;
...
const char *response_to_special_client_request(char *abuf,int *len2) {
  ...
  {
    const char cmd[]="remote";
    if (0==strncmp(abuf,cmd,strlen(cmd))) {
      char cmd2[50];
      if (1==sscanf(abuf+strlen(cmd),"%s",cmd2)) {
	if (0==strncmp(cmd2,play_str,strlen(play_str))) {
	  req_play=true;
	  *len2=strlen(play_str);
	  return play_str;
	}
	if (0==strncmp(cmd2,pause_str,strlen(pause_str))) {
	  req_pause=true;
	  *len2=strlen(pause_str);
	  return pause_str;
	}
      }
      *len2=0;
...
void process_remote_buttons(int s) {
  ...
  if (q==REMOTE_SPOT || (qq & 4) || req_pause) { //pause
    action_prime=STOP;
    req_pause=false;
  } else if (q==REMOTE_FORWARD || (qq & 1) || req_play) { //start
...
int pavlovian_mode_with_tests_policy() { 
  curr_act_prob=1;
  int action=FORWARD;
  if ( hit_decay >= 1 || (last_act==COUNTERCLOCKWISE && predictions[0] > prediction_threshold)) { // force some turning away
    action=COUNTERCLOCKWISE;
    curr_act_prob=1;
    testing=0;
  } else if (testing) { // continue testing
    action=FORWARD;
    curr_act_prob=1;
  } else if (predictions[0] > prediction_threshold) { // probability of testing
    if (uniform_random() < testing_probability) {
      action=FORWARD;
      curr_act_prob=testing_probability;
      testing=1;
      play_explore();
    } else {
      action=COUNTERCLOCKWISE;
      curr_act_prob=1-testing_probability;
      testing=0;
    }
  }  else if (last_act==COUNTERCLOCKWISE) {
    if (uniform_random() > beta_drive_after_cleared) {
...
const  float expected_switches_per_second=5;
const  float T=decisions_per_second/expected_switches_per_second;
const  int num_actions=2;
const  float k_switch=log((T-1)*(num_actions-1));
...
int pavlovian_modeless_policy() {
  ...
  float k1=k_switch*4,k2=k_switch;
  float k3=k_switch;
  float k4=k_switch*3;
  float pref[3],weights[4];
  pref[0]=exp(-k1*(p0-0.5) + k2*(last_act==FORWARD) + k4*mem_trace_turning);
  pref[1]=exp(k3*(last_act==COUNTERCLOCKWISE));
  //pref[2]=exp(k2*(last_act==CLOCKWISE));
  //  printf("\nmodeless prefs: %4.3f %4.3f %d : [%4.3f %4.3f]\n",pref[0],pref[1], last_act, k1, p0);
...
float alphaR=.000001,alphaU=.00001,alphaV=.00001;
float lambdaAC=0.8;
...
void ActorCriticInit() {
  ...
  u[1]=-KS*4;      // x[1] = prediction 
  u[2]=KS;         // x[2] = last_act==FORWARD,
  u[3]=KS;         // x[3] = last_act==COUNTERCLOCKWISE
  u[4]=KS;         // x[4] = 3 second memory trace of x[3] 
                   // (x[4]~=1 should strongly prefer going forward)
...
double piAC(int actK, float x[]) {
  float p[2];
  p[0]= exp(u[0]*x[0] + u[1]*x[1] + u[2]*x[2] + u[4]*x[4] ); // Boltzman
  p[1]= exp(u[3]*x[3]);
  //p[2]= exp(u[2]*x[4]);
  //printf("Actor prefs: %4.3f %4.3f \t %4.3f %4.3f %4.3f\n",p[0],p[1],u[0],u[1],u[2]);
  //  printf("Actor-Critic prefs: %4.3f %4.3f : (%4.3f %4.3f) (%4.3f %4.3f) \n\n",p[0],p[1],u[0],x[0],u[1],x[1]);
...
double grad_X(int actK,int uI, float x[], float pi0x,float pi1x) {
...
void ActorCriticUpdate(float reward, int actK) {
  // This is set when selecting the action according to the policy
  //xACprime[0]=1;
  //xACprime[1]=predictions[0];
  //xACprime[2]=last_act==FORWARD;
  //xACprime[3]=last_act==COUNTERCLOCKWISE;
  ...
  //actor
  float pi0x=piAC(0,xAC);
  float pi1x=1-pi0x;
  for (int i=0;i<nActor;i++) {
    eU[i] = lambdaAC* eU[i] + grad_X(actK,i,xAC,pi0x,pi1x);
    u[i] += alphaU * delta * eU[i];
  }

  //printf("r=%4.3f rbar= %4.3f delta=%4.3f u[0]=%4.3f u[1]=%4.3f\n",reward, rbar,delta, u[0], v[0]);
...
int real_policy() {
  ...
  if (MODES==policy_type)
    return pavlovian_mode_with_tests_policy();
...
int agentSelectAction(int s) {
  ...
  mem_trace_turning=(last_act==COUNTERCLOCKWISE)*(1-mem_trace_turning_gamma) +mem_trace_turning_gamma*mem_trace_turning; 
...
//float posthoc_sigmoid(float x){ // centered on (0.5,0.5)
...
float make_prediction(int x[],int demon){
  ...
  // clipped to [0,1] -- only probability-like predictions
  lprediction = lprediction < 0 ? 0 : lprediction > 1 ? 1 : lprediction;
  return lprediction;
}
...
void updateLearning(int obs,int act){
  rs[0] =obs > 0 ? 1 : 0;
  gammas[0]=rs[0]>0 ? 0 : 1-3./100.; // ... gamma is now 0.97 = 1 second in exp.
  Is[0]=1;
  ...
  /* 
  // Importance selection and (perhaps ok) rhos
  ...
  */
  ...
  int bump= (obs&3) > 0;
  // going forward is negative distance on the Create2.
  // going forward is positive distance on the Create.
  reward=-dist - 20*bump;// - 100 * rs[0];
  if (ACTOR_CRITIC==policy_type) {
...
void updateLogfile(int obs, int act, int pktNum){
  ...
  {//logging
    int c=sprintf(logstring,"%9.3f %9.3f %d %d %4.3f %4.3f %d %d %d %d %d %d %d",netTime, updateTime, iteration, cycles, rs[0], gammas[0], obs,act, dist, pktNum, action_prime, mode_duration,vid_count);
    for (int i=0;i<DEMONS;i++) 
      c+=sprintf(logstring+c," %4.3f",predictions[i]);
    cycles++;
    if (action_prime!= STOP)
      iteration++;
    c+=sprintf(logstring+c," %d %d %d %d %d %4.3f",era, learn_prediction, use_prediction, policy_type, testing, hit_decay);
    for (int k=0;k<data_size;k++) {
      c+=sprintf(logstring+c," %d",pkt_data[k]);
    }
    //loc += sprintf(start_comment+loc," deltaAC avgR R V Vplus u0 u1 u2");
    c+=sprintf(logstring+c," %f %f %f %f %f %f %f %f %f %f", r_delta, rbar, reward, v_now, v_plus, u[0], u[1], u[2],u[3],u[4] );
...
void init_logging() {
  ...
  loc += sprintf(start_comment+loc,"#Labels netTime updateTime iteration cycle r gamma obs act dist pktNum action_prime mode_duration vid_count");
  for (int i=0;i<DEMONS;i++)
    loc += sprintf(start_comment+loc," prediction_%d",i);
  loc += sprintf(start_comment+loc," era learn_prediction use_prediction policy_type testing hit_decay");
  ...
  loc += sprintf(start_comment+loc," deltaAC avgR R V Vplus u0 u1 u2 u3 u4");
