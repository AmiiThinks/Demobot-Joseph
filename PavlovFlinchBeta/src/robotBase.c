/* 
   ...
   Modified by joseph - June 22, 2015 - Added -create2 
...
int create2_robot=0;           // initialize robot communication at 115k
...
void getPktValues(int myPktNum, int data[]) {
  int i=0;
  int p = (myPktNum + M - 1) % M;
  data[i++]=sCliffL[p];
  data[i++]=sCliffR[p];
  data[i++]=sCliffFL[p];
  data[i++]=sCliffFR[p];
...
int wheeldrops(int p) {
  ...
  // The sensor on the castor is triggering and causing the robot's motion to stutter.  add back as needed.
...
int bmain(int argc, char *argv[]) {
  ...
  if ((argc > 1) && (0==strncmp("-create2",argv[1],10))) {
    create2_robot=1;
    printf("Initialization at 115k for a create2.\n");
  } else {
    printf("Initialization for a create.\n");
  }
  ...
  lockWriteAction(a);
  agentUpdateAfterAction(s,a,myPktNum); //just initialization
  prevPktNum = myPktNum;
  rewardReport = 0;
  while (TRUE) { // main agent loop
    ...
    // agent Learning
    agentUpdateAfterAction(sprime,aprime,myPktNum);
...
void reflexes() {
  ...
  */
  ...
  // Send actions only on even timesteps - unless they have changed.

  // This change measurably improves the robot's reponsiveness to a
  // bumper press from 12*30ms= 360ms to 4 * 30ms = 120ms. I think the
  // Create2s have "smarter" wheel motors that introduce delays when
  // switching and might stuggle to keep up with commands sent every
  // 15ms.  
  if (p > 0 && p%2==1 && sDrive[p]==sDrive[p-1])
    return;
  takeAction(sDrive[p]); 
