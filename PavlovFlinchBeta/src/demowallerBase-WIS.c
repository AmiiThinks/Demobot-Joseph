...
const float u_0 = 10000; //NUMACTIVEFEATURES*100;                   // initial usage
const float eta = 1./u_0; /// NUMACTIVEFEATURES;   // recency-weighting factor
...
void init_learning() {
  ...
  printf("From init learning: uu[0]: %lf\n", uu[0]);  
...
float make_prediction(int x[],int demon){
  ...
  //return posthoc_sigmoid(lprediction);
  return lprediction;
}
...
void WISTDupdate(int demon) {
  ...
  if (learn_prediction) {
    ...
    for (int i=0;i< NN; i++) {
      ...
      + (r + gamma*prednext - predprev)*et[i+offset] \
...
void WISGTDupdate(int demon) {
  ...
  if (learn_prediction) {
    ...
    for (int i=0;i< NN; i++) {
      ...
      // update of the main weight vector
      w[i+offset]     += alpha*(r + gamma*prednext - pred)*et[i+offset] \
  ...
  if (learn_prediction) {
    ...
    for (int i=0;i< NN; i++) {
      ...
      et[i+offset]     = rho*(M*xnow[i] \
                              + lambdaprev*gammaprev*et[i+offset]);
      // update of the main weight vector
      float update = alpha*(r + gamma*prednext - pred)*et[i+offset];
...
const float min_prob_action=0.0;
void updateLearning(int obs,int act){
  rs[0] =obs > 0 ? 1 : 0;
  ...
  if (last_act !=FORWARD)
    rhos[0]=0; // target
  else if (last_act_prob<min_prob_action) {
    printf("\n\nTerminating prediction, too big rho!!\n\n");
    Is[0] = 0;
    gammas[0] = fmin(gammas[0], last_act_prob);
...
void updateLogfile(int obs, int act, int pktNum){
  ...
  {//logging
    ...
    c+=sprintf(logstring+c," %d %d %d %d %d %4.3f %4.3f %4.3f %4.3f",era, learn_prediction, use_prediction, policy_type, testing, hit_decay, rhos[0], Is[0], et[0]);
...
void init_logging() {
  ...
  //loc += sprintf(start_comment+loc," era learn_prediction use_prediction policy_type curr_act_prob rho");
    loc += sprintf(start_comment+loc," era learn_prediction use_prediction policy_type testing hit_decay rho I elig[0]");
