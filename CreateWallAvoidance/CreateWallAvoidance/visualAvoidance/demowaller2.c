...
void process_image(const void *p, int size){ // implemented by the client now.
  ...
  for (int i=0;i<MM;i++) {
    selected_pixels[vid_count%2][i]=((unsigned char*) p) [pixelinds[i]] >> 3;
...
void init_logging() {
  ...
  sprintf(logfilename,"mkdir ../logfiles/demoWaller2-y%d-m%02d-d%02d-h%02d-m%02d-s%02d.src",
          (tB->tm_year+1900),(1+tB->tm_mon),tB->tm_mday,
           tB->tm_hour,tB->tm_min,tB->tm_sec);
  system(logfilename);
  sprintf(logfilename,"cp Makefile *.c *.h ../logfiles/demoWaller2-y%d-m%02d-d%02d-h%02d-m%02d-s%02d.src",
          (tB->tm_year+1900),(1+tB->tm_mon),tB->tm_mday,
           tB->tm_hour,tB->tm_min,tB->tm_sec);
  system(logfilename);
  sprintf(logfilename,"../logfiles/demoWaller2-y%d-m%02d-d%02d-h%02d-m%02d-s%02d.logfile",
          (tB->tm_year+1900),(1+tB->tm_mon),tB->tm_mday,
           tB->tm_hour,tB->tm_min,tB->tm_sec);
