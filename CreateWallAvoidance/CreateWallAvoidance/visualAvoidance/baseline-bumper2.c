...
int bmain(int argc, char *argv[]) {
  ...
  */
  ...
  lockWriteAction(a);
  agentUpdate(s,a,p);//myPktNum); //just initialization
  prevPktNum = myPktNum;
  rewardReport = 0;
...
void setupSerialPort(const char serialPortName[]) {
  ...
  // Removing the following lines helped enable serial port communications.
  // It does not always seem to be needed, but may help with the
  // silver USB-serial adaptors.
