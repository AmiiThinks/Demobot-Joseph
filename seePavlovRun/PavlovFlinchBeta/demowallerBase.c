//Call the AC init and update.

// determine why the default AC code is rubbish - maybe the lack of init.
...
const char logfile_prefix[]="../logfiles/demowallerBase";
...
int pavlovian_mode_policy() { 
  ...
  if ( hit_decay >=1 || predictions[0] > prediction_threshold) {
    action=COUNTERCLOCKWISE;
    curr_act_prob=1;
  }  else if (last_act==COUNTERCLOCKWISE &&  uniform_random() > beta_drive_after_cleared) {
    action=COUNTERCLOCKWISE;
    curr_act_prob=1-beta_drive_after_cleared;
  } else {
    action=FORWARD;
...
void updateLearning(int obs,int act){
  ...
  if (last_act !=FORWARD)
    ...
    rhos[0]=0; // GQ with I=0 and no action-dependent features is rho=0
