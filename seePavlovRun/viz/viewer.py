...
def throw(image):
    ...
    if img is None:
        ...
        img = ax.imshow(image,interpolation='none',origin='lower')
    else:
        img.set_data(image)
    P.pause(.01)
    P.draw()
...
def reduced_image(k=100,labels=fake_label2):
    return ["image",yuv.extract_vals_500,yuv.offsets500[:k],.505,labels]

def nonan_float(x):
    v=float(x)
    return 0 if P.isnan(v) else v
...
def large_movie(parser=big_image):#parser=yuv.large_parser,start_time=0.3,end_time=30):
    ...
    while True:
        ...
        #if len(x)< 110:
        #    continue
        items=[(n,nonan_float(x[a])) for n,a in name_inds]
        rs=[items[1][1]]+rs[:99]
        ...
        if True:
            ...
            data.append(rs)
            name=yuv.ite(rs[0]==0,"(No Bump)" , "(Bump)")
            ...
            image=image_annotate(rgb,data,showPred=preds[0],isBump=rs[0],showAct=acts[0]);
            #return image
        throw(image)
            #count+=1
    #f.close()
