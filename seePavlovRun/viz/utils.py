...
def list_objs(file='bindata'):
    import shelve
    return shelve.open(file).keys()

def real_range(start,end,step):
    return [start+i*step for i in xrange(int((end-start)/step)) ]
...
def str_dict(dic):
    out="{ "+ "\n  ".join([ strcap("%s : %s "%(k,v)) for k,v in dic.items()])+"\n}"
    return out
...
def entropy(name_counts,names=1):    
    if names!=0:
        counts=map(lambda x:x[1],name_counts);
    else:
        counts=name_counts
    counts=filter(lambda x: x>0,counts)
    total=0.0+sum(counts)
    normalized=map(lambda x: x/total, counts);
    return -sum(map(lambda x: x*math.log(x),normalized))
...
def argmax(seq):
    return arg(max,zip(range(len(seq)),seq))
...
def find_python_dependencies():
    ...
    for x in deps.keys():
        aaa=[y for y in deps[x] if y in deps.keys()]
        deps[x]=uniq(aaa)
