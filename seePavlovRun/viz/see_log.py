...
def extract_from_explore_bumps(d,l):
    ...
    P.plot(range(-40,0), mn)
    P.plot(range(-40,0),[math.exp(x/20.) for x in range(-40,0)])
    return raw
...
def grab_data_labels_nofeats(filename):
    ...
    last=ls.index("im000")
    print last,ls[last]
    d=P.loadtxt(filename,usecols=range(0,last))
    return d,ls[:last]
